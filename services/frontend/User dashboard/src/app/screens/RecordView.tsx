...
export function RecordView({
  ...
  const [emotionResult, setEmotionResult] =
    useState<EmotionAnalysisRead | null>(null);

  // 분석 데이터가 아직 부족한 경우 사용자에게 보여줄 안내
  const [analysisNotice, setAnalysisNotice] = useState<string | null>(null);

  const [minuteValues, setMinuteValues] = useState<
    Record<MinuteField, number | null>
  >({
    sleep_minutes: null,
    rest_minutes: null,
  ...
  const [editedByUser, setEditedByUser] = useState<
    Partial<Record<NullableMetricKey, boolean>>
  >({});
  ...
  const [existingRecord, setExistingRecord] =
    useState<BehavioralRecordRead | null>(null);
  ...
  function handleHourChange(field: MinuteField, rawHour: string) {
    ...
    setMinuteValues(v => ({
      ...v,
      [field]:
        Math.min(24, Math.max(0, h)) * 60 + curM,
    }));
  ...
  async function handleNext() {
    ...
    try {
      if (alreadyRecordedToday && existingRecord) {
        // 이 화면이 다루는 4개 필드만 덮어쓰고,
        // 나머지 6개 값과 그 메타데이터는 서버에 있던 값을 그대로 돌려보낸다.
        // (예: 모바일 백그라운드 동기화로 들어온 steps/active_minutes를 웹 수정으로 날려버리지 않기 위함)

        const minuteMeta = buildFieldMeta(
          minuteValues,
          editedByUser
        );

        const source_by_field = {
          ...existingRecord.source_by_field,
        };

        const coverage_by_field = {
          ...existingRecord.coverage_by_field,
        };

        for (const [field] of MINUTE_FIELD_LABELS) {
          if (!editedByUser[field]) continue;

          // 안 고친 필드는 기존 source(manual 등)를
          // health_platform으로 되돌리면 안 됨
          source_by_field[field] =
            minuteMeta.source_by_field[field];

          coverage_by_field[field] =
            minuteMeta.coverage_by_field[field];
        }

        const updated = await updateBehavioralRecord(
          todayDateString(),
          {
      ...
      const { source_by_field, coverage_by_field } =
        buildFieldMeta(allValues, editedByUser);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Recovery Report 생성 시
          // 아직 분석에 필요한 데이터가 부족한 경우
          // 기록 저장 자체는 성공한 것으로 처리한다.
          if (
            err instanceof ApiError &&
            (err.status === 409 || err.status === 503)
          ) {
            setAnalysisNotice(
              "기록은 저장했어요. 회복 리포트는 분석할 데이터가 더 쌓이면 준비할게요."
            );
          } else {
            throw err;
          }
        ...
        // Risk Evaluation 생성 시
        // baseline 부족 등의 이유로 분석할 수 없는 경우
        // 기록 저장 자체는 성공한 것으로 처리한다.
        if (
          err instanceof ApiError &&
          err.status === 409
          ...
          setAnalysisNotice(
            "기록은 저장했어요. 아직 분석할 데이터가 부족해요. 데이터가 더 쌓이면 생활 리포트를 준비할게요."
          );
        } else {
          // 데이터 부족이 아닌 실제 API 오류는
          // 정상적인 성공으로 숨기지 않는다.
          throw err;
        }
      }
      ...
      <section className="mx-auto max-w-3xl">
        ...
        <div className="mt-9 divide-y divide-border border-y border-border">
          <SummaryLine
            label="수면"
            value={
              formatMinutes(minuteValues.sleep_minutes) ||
              "기록 없음"
            }
          />
          ...
          <SummaryLine
            label="휴식"
            value={
              formatMinutes(minuteValues.rest_minutes) ||
              "기록 없음"
            }
          />
  ...
  return (
    <section className="mx-auto max-w-3xl">
      ...
      <h1 className="mt-4 font-serif text-[31px] tracking-[-.05em]">
        {editing
          ? "기록을 다시 살펴볼까요?"
          : "오늘 하루를 남겨볼까요?"}
      </h1>
      ...
      <p className="mt-3 text-sm leading-6 text-muted-foreground">
        정확하지 않아도 괜찮아요. 기억나는 만큼만 적어 주세요.
      </p>

      {alreadyRecordedToday && step === 1 && (
        <p className="mt-3 text-xs text-[#8d5541]">
          오늘 기록은 이미 저장돼 있어요. 값을 고치면 새로 저장할게요.
        </p>
      )}

      <div className="mt-9 flex items-center gap-3 text-xs">
        <span className="text-[#68796b]">
          01 생활
        </span>
        ...
        {step === 1 ? (
          ...
          <>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              적어주신 내용을 바탕으로 오늘의 감정 상태를 분석해드려요.
              감정을 직접 고르지 않아도 괜찮아요.
            </p>
